...
- 型号已知则写具体型号；
- 型号未知但只知道类别时，写 `3P连接器`、`6P连接器`、`DB44公头` 等，不编造采购型号；
...
4. 连接点1/连接点2方向没有写反；
5. 节点号1 与连接点2之间存在空白分隔列；
6. 多针并联已逐针展开；
7. Field/High Current Field 极性与 IN/OUT 正确；
8. 线型来自用户旧表或明确说明；
9. 电气属性来自官方 PDF 或用户资料；
10. 不存在人为镜像编号；
11. 每个接线 Sheet 底部有标准说明行；
12. Excel 可正常打开，无公式错误；
13. 配色简洁，无多余颜色。

## 12. 标准交付话术

> 已按统一接线表标准生成：简洁黑白/浅灰配色、两端中间增加空白分隔列、保留连接器型号 Sheet，并在最后一列补充官方资料中的电气属性。  
