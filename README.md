...
| [mineru](mineru/SKILL.md) | 优先用于 PDF、扫描件、图片与 Office 文档的解析、OCR、表格/公式读取、页/块定位和渐进式阅读；**Skill 只负责调用规则，实际解析需单独安装 MinerU 4.x 本体**。 |
...
| **mineru** | 官方 Skill 引用 / 适配 | **MinerU Team / OpenDataLab** | [opendatalab/MinerU](https://github.com/opendatalab/MinerU) | MinerU Open Source License（基于 Apache 2.0 并附加条款）；完整条款见 `mineru/LICENSE.md` |
...
- `mineru` 基于上游官方 Agent Skill 适配；仓库不包含 MinerU 解析器本体，运行时需另行安装 `mineru>=4.0,<5`。来源与附加许可条件见 [`mineru/THIRD_PARTY_NOTICES.md`](mineru/THIRD_PARTY_NOTICES.md) 与 [`mineru/LICENSE.md`](mineru/LICENSE.md)。
