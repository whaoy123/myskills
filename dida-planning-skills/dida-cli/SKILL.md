...
- Put writes in a JSON plan and run `plan --input <file>` first. It is dry-run by default and validates the whole plan, including create keys and `@key` references, before any API access; use `--apply` only after resolving IDs and confirming the write scope.
