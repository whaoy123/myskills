...
默认只输出一句话格式（未校准/校准后各一行）：

- 未校准：`误差为 ±(a% × 读数 + b_v) RMS，FS = <输入满量程及口径>`（最坏值），并附 `RSS ±(ar% × 读数 + br_v) RMS`；
- 已校准：`误差为 ±(ac% × 读数 + bc_v) RMS`（只含校准无法消除的温漂、非线性、校准源不确定度等剩余项）。

其中：

- `b_v = b × 输入FS`，固定项直接换算成绝对值，不再写百分比；
- 口径默认 RMS，输入按峰值定义时注明“峰值”（RMS 换算峰值 ×√2）；
- 直流通道省略 RMS 后缀，FS 写“300 V DC / 28 V DC / ±7 A / ±120 mA”等；固定项按被测端物理单位给绝对值（V / mA / A）。
- “输入FS”指被测信号端满量程（POR 600 VAC / CT 1 A / FieldI ±7 A），不是隔离放大器输入端或 ADC 满量程；链路内部节点的误差先折算回被测输入端再合并。

示例：`误差为 ±(0.44% × 读数 + 60 mV) RMS，FS = 600 VAC RMS（最坏）；RSS ±(0.28% × 读数 + 42 mV) RMS`。

用户追问细节时，再按来源 c、d、e、f… 展开，每项说明“由什么产生、误差为多少、折算到输入端多少、校准后是否保留”：

- 来源 c：<器件/参数>，产生误差 <局部值>，折算到输入端 <值>，校准后保留：是/否；
- 来源 d：……
- 来源 e：……
- 来源 f：……
