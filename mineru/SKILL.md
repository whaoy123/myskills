...
PDF page selection is shared across CLI, Doclib, API, Gradio and Python. See the [page-range syntax and historical result compatibility](https://github.com/opendatalab/MinerU/blob/master/docs/next/page-ranges.md). New requests use the current syntax; stored positive page ranges using ASCII `~` remain readable without rebuilding Doclib caches. Fullwidth `～` and negative page-number notation are not supported.
...
This copy is based on the official MinerU skill from [opendatalab/MinerU](https://github.com/opendatalab/MinerU). The MinerU runtime is not vendored here and must be installed separately. See [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) and [LICENSE.md](LICENSE.md) for source and license information.
